   ...
   public class IkConstraintTimeline extends CurveTimeline
   {
      ...
      override public function apply(param1:Skeleton, param2:Number, param3:Number, param4:Vector.<Event>, param5:Number, param6:Boolean, param7:Boolean) : void
      {
         ...
         if(param3 < frames[0])
         {
            if(param6)
            {
               _loc9_.mix = _loc9_.data#2.mix;
               _loc9_.bendDirection = _loc9_.data#2.bendDirection;
            }
            return;
         }
         if(param3 >= frames[int(frames.length - 3)])
         {
            if(param6)
            {
               _loc9_.mix = _loc9_.data#2.mix + (frames[frames.length + -2] - _loc9_.data#2.mix) * param5;
               _loc9_.bendDirection = !!param7 ? _loc9_.data#2.bendDirection : int(frames[frames.length + -1]);
            }
            else
            {
         ...
         if(param6)
         {
            _loc9_.mix = _loc9_.data#2.mix + (_loc10_ + (frames[_loc12_ + 1] - _loc10_) * _loc11_ - _loc9_.data#2.mix) * param5;
            _loc9_.bendDirection = !!param7 ? _loc9_.data#2.bendDirection : int(frames[_loc12_ + -1]);
         }
         else
         {
