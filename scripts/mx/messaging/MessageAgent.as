   ...
   public class MessageAgent extends EventDispatcher implements IMXMLObject
   {
      ...
      public function set destination(param1:String) : void
      {
         ...
         if(_destination != param1)
         {
            ...
            if(Log.isInfo())
            {
               _log.info("\'{0}\' {2} set destination to \'{1}\'.",id#2,_destination,_agentType);
            }
         }
      }
      ...
      public function acknowledge(param1:AcknowledgeMessage, param2:IMessage) : void
      {
         ...
         if(Log.isInfo())
         {
            _log.info("\'{0}\' {2} acknowledge of \'{1}\'.",id#2,param2.messageId,_agentType);
         }
         ...
         if(param1.headers#2[AcknowledgeMessage.ERROR_HINT_HEADER])
         {
            delete param1.headers#2[AcknowledgeMessage.ERROR_HINT_HEADER];
         }
      ...
      public function fault(param1:ErrorMessage, param2:IMessage) : void
      {
         if(Log.isError())
         {
            _log.error("\'{0}\' {2} fault for \'{1}\'.",id#2,param2.messageId,_agentType);
         }
         _ignoreFault = false;
         configRequested = false;
         if(param1.headers#2[ErrorMessage.RETRYABLE_HINT_HEADER])
         {
            delete param1.headers#2[ErrorMessage.RETRYABLE_HINT_HEADER];
         }
         if(clientId == null)
         {
            if(param1.clientId != null)
      ...
      public function channelDisconnectHandler(param1:ChannelEvent) : void
      {
         if(Log.isWarn())
         {
            _log.warn("\'{0}\' {1} channel disconnected.",id#2,_agentType);
         }
         setConnected(false);
      ...
      public function channelConnectHandler(param1:ChannelEvent) : void
      {
         ...
         if(mx_internal::needsConfig)
         {
            if(Log.isInfo())
            {
               _log.info("\'{0}\' {1} waiting for configuration information.",id#2,_agentType);
            }
         ...
         else
         {
            if(Log.isInfo())
            {
               _log.info("\'{0}\' {1} connected.",id#2,_agentType);
            }
            setConnected(true);
            dispatchEvent(param1);
         }
      ...
      public function channelFaultHandler(param1:ChannelFaultEvent) : void
      {
         if(Log.isWarn())
         {
            _log.warn("\'{0}\' {1} channel faulted with {2} {3}",id#2,_agentType,param1.faultCode,param1.faultDetail);
         }
