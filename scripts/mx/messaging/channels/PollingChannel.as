package mx.messaging.channels
{
   ...
   public class PollingChannel extends Channel
   {
      ...
      mx_internal function stopPolling() : void
      {
         if(Log.isInfo())
         {
            _log.info("\'{0}\' channel polling stopped.",id#2);
         }
         if(mx_internal::_timer != null)
         {
            mx_internal::_timer.stop();
      ...
      protected function internalPoll(param1:Event = null) : void
      {
         ...
         if(!mx_internal::pollOutstanding)
         {
            if(Log.isInfo())
            {
               _log.info("\'{0}\' channel requesting queued messages.",id#2);
            }
         ...
         {
            _log.info("\'{0}\' channel waiting for poll response.",id#2);
         }
      }
      ...
      {
         return mx_internal::_timer == null ? 0 : Number(mx_internal::_pollingInterval);
      }
      ...
      protected function startPolling() : void
      {
         if(_pollingEnabled)
         {
            if(Log.isInfo())
            {
               _log.info("\'{0}\' channel polling started.",id#2);
            }
            _shouldPoll = true;
            poll();
         }
