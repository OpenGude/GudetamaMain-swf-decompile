package mx.messaging
{
   ...
   public class Channel extends EventDispatcher implements IMXMLObject
   {
      ...
      private function calculateEndpoint() : void
      {
         ...
         if(_loc2_.length == 0)
         {
            _loc1_ = URLUtil.getFullURL(LoaderConfig.url#2,_loc1_);
         }
         ...
         if(Log.isInfo())
         {
            _log.info("\'{0}\' channel endpoint set to {1}",id#2,_endpoint);
         }
      }
      ...
      public function setCredentials(param1:String, param2:MessageAgent = null, param3:String = null) : void
      {
         ...
         if(connected && _loc4_ && param1 != null)
         {
            ...
            if(param3 != null)
            {
               _loc5_.headers#2[CommandMessage.CREDENTIALS_CHARSET_HEADER] = param3;
            }
      ...
      {
         var _loc2_:String = FlexClient.getInstance().id#2;
      ...
      protected function flexClientWaitHandler(param1:PropertyChangeEvent) : void
      {
         ...
         {
            _loc2_ = param1.source#2 as FlexClient;
      ...
      public function send(param1:MessageAgent, param2:IMessage) : void
      {
         ...
         if(Log.isDebug())
         {
            _log.debug("\'{0}\' channel sending message:\n{1}",id#2,param2.toString());
         }
         param2.headers[AbstractMessage.ENDPOINT_HEADER] = id#2;
      ...
      protected function connectSuccess() : void
      {
         ...
         if(Log.isInfo())
         {
            _log.info("\'{0}\' channel is connected.",id#2);
         }
      ...
      protected function disconnectSuccess(param1:Boolean = false) : void
      {
         ...
         if(Log.isInfo())
         {
            _log.info("\'{0}\' channel disconnected.",id#2);
         }
      ...
      protected function connectFailed(param1:ChannelFaultEvent) : void
      {
         ...
         if(Log.isError())
         {
            _log.error("\'{0}\' channel connect failed.",id#2);
         }
      ...
      public final function connect(param1:ChannelSet) : void
      {
         ...
         else if(!_connecting)
         {
            ...
            if(FlexClient.getInstance().id#2 == null)
            {
      ...
      private function failover() : void
      {
         ...
         if(_failoverIndex + 1 <= failoverURIs.length)
         {
            ...
            if(Log.isInfo())
            {
               _log.info("\'{0}\' channel attempting to connect to {1}.",id#2,endpoint);
            }
         ...
         else
         {
            if(Log.isInfo())
            {
               _log.info("\'{0}\' channel has exhausted failover options and has reset to its primary endpoint.",id#2);
            }
            resetToPrimaryURI();
         }
      }
      ...
      protected function disconnectFailed(param1:ChannelFaultEvent) : void
      {
         ...
         if(Log.isError())
         {
            _log.error("\'{0}\' channel disconnect failed.",id#2);
         }
         if(reconnecting)
         {
            resetToPrimaryURI();
