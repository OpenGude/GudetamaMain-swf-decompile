package mx.messaging
{
   ...
   public class AbstractConsumer extends MessageAgent
   {
      ...
      protected function buildUnsubscribeMessage(param1:Boolean) : CommandMessage
      {
         ...
         if(param1)
         {
            _loc2_.headers#2[CommandMessage.PRESERVE_DURABLE_HEADER] = param1;
         }
         return _loc2_;
      }
      ...
      public function subscribe(param1:String = null) : void
      {
         ...
         if(Log.isInfo())
         {
            _log.info("\'{0}\' {1} subscribe.",id#2,_agentType);
         }
      ...
      protected function startResubscribeTimer() : void
      {
         if(_shouldBeSubscribed && _resubscribeTimer == null)
         {
            if(_resubscribeAttempts != 0 && _resubscribeInterval > 0)
            {
               if(Log.isDebug())
               {
                  _log.debug("\'{0}\' {1} starting resubscribe timer.",id#2,_agentType);
               }
      ...
      public function unsubscribe(param1:Boolean = false) : void
      {
         ...
         if(subscribed)
         {
            ...
            if(Log.isInfo())
            {
               _log.info("\'{0}\' {1} unsubscribe.",id#2,_agentType);
            }
         ...
         if(_loc2_ is CommandMessage)
         {
            ...
            switch(_loc3_.operation)
            {
               ...
               default:
                  if(Log.isWarn())
                  {
                     _log.warn("\'{0}\' received a CommandMessage \'{1}\' that could not be handled.",id#2,CommandMessage.getOperationAsString(_loc3_.operation));
                  }
            }
            return;
         }
      ...
      override public function fault(param1:ErrorMessage, param2:IMessage) : void
      {
         ...
         if(param1.headers#2[ErrorMessage.RETRYABLE_HINT_HEADER])
         {
      ...
      protected function stopResubscribeTimer() : void
      {
         if(_resubscribeTimer != null)
         {
            if(Log.isDebug())
            {
               _log.debug("\'{0}\' {1} stopping resubscribe timer.",id#2,_agentType);
            }
      ...
      override public function acknowledge(param1:AcknowledgeMessage, param2:IMessage) : void
      {
         ...
         if(!param1.headers#2[AcknowledgeMessage.ERROR_HINT_HEADER] && param2 is CommandMessage)
         {
            ...
            switch(_loc4_)
            {
               case CommandMessage.UNSUBSCRIBE_OPERATION:
                  if(Log.isInfo())
                  {
                     _log.info("\'{0}\' {1} acknowledge for unsubscribe.",id#2,_agentType);
                  }
               ...
               case CommandMessage.SUBSCRIBE_OPERATION:
                  ...
                  if(Log.isInfo())
                  {
                     _log.info("\'{0}\' {1} acknowledge for subscribe. Client id \'{2}\' new timestamp {3}",id#2,_agentType,param1.clientId,_timestamp);
                  }
      ...
      protected function resubscribe(param1:TimerEvent) : void
      {
         ...
         if(Log.isDebug())
         {
            _log.debug("\'{0}\' {1} trying to resubscribe.",id#2,_agentType);
         }
