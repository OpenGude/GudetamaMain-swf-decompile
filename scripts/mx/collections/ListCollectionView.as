   ...
   public class ListCollectionView extends Proxy implements ICollectionView, IList, IMXMLObject
   {
      ...
      private function removeItemsFromView(param1:Array, param2:int, param3:Boolean = true) : void
      {
         ...
         if(param3 && _loc4_.length > 0)
         {
            ...
            _loc9_.location#2 = !localIndex || _loc4_.length == 1 ? int(_loc5_) : -1;
            _loc9_.items = _loc4_;
            dispatchEvent(_loc9_);
         }
      }
      ...
      private function handlePropertyChangeEvents(param1:Array) : void
      {
         ...
         var _loc11_:int = 0;
         var _loc12_:int = 0;
         var _loc13_:CollectionEvent = null;
         ...
         if(sort || filterFunction != null)
         {
            ...
            while(_loc5_ < param1.length)
            {
               if((_loc8_ = param1[_loc5_]).target)
               {
                  ...
                  _loc10_ = _loc8_.target != _loc8_.source#2;
               }
               else
               {
                  _loc9_ = _loc8_.source#2;
                  _loc10_ = false;
               }
               _loc11_ = 0;
            ...
            while(_loc7_ < _loc2_.length)
            {
               ...
               {
                  if(_loc2_[_loc7_] == _loc3_[_loc12_].item)
                  {
                     _loc6_.push(_loc3_[_loc12_].event);
                  }
                  _loc12_++;
               }
               _loc7_++;
         ...
         if(_loc2_.length > 0)
         {
            (_loc13_ = new CollectionEvent(CollectionEvent.COLLECTION_CHANGE)).kind = CollectionEventKind.UPDATE;
            _loc13_.items = _loc2_;
      ...
      mx_internal function getBookmarkIndex(param1:CursorBookmark) : int
      {
         ...
         var _loc2_:ListCollectionViewBookmark = ListCollectionViewBookmark(param1);
         if(_loc2_.viewRevision != revision)
         {
            if(_loc2_.index < 0 || _loc2_.index >= length || getItemAt(_loc2_.index) != _loc2_.value)
            {
               _loc2_.index = getItemIndex(_loc2_.value);
            }
            _loc2_.viewRevision = revision;
         }
         return _loc2_.index;
      }
         ...
         else
         {
            switch(param1.kind)
            {
               case CollectionEventKind.ADD:
                  addItemsToView(param1.items,param1.location#2);
                  break;
               case CollectionEventKind.RESET:
                  reset();
                  break;
               case CollectionEventKind.REMOVE:
                  removeItemsFromView(param1.items,param1.location#2);
                  break;
               case CollectionEventKind.REPLACE:
                  replaceItemsInView(param1.items,param1.location#2);
                  break;
...
class ListCollectionViewBookmark extends CursorBookmark
{
   ...
   {
      return mx_internal::view.getBookmarkIndex(this);
   }
}
...
class ListCollectionViewCursor extends EventDispatcher implements IViewCursor
{
   ...
   private function collectionEventHandler(param1:CollectionEvent) : void
   {
      ...
      {
         case CollectionEventKind.ADD:
            if(event.location#2 <= currentIndex)
            {
         ...
         case CollectionEventKind.REMOVE:
            ...
            else if(event.location#2 == currentIndex)
            {
               if(currentIndex < view.length)
               {
                  try
