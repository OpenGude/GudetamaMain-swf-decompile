package starling.animation
{
   ...
   public class Tween extends EventDispatcher implements IAnimatable
   {
      ...
      private static const toRad:Number = 0.017453292519943295;
      ...
      public function synchronize(param1:Tween) : void
      {
         _currentTime = param1._currentTime;
         _currentCycle = param1._currentCycle;
      }
      ...
      public function forceFinish() : void
      {
         ...
         if(_repeatCount > 1)
         {
            _loc1_ = _repeatCount - 1;
            _currentCycle += _loc1_;
            _repeatCount = 1;
         }
         advanceTime(_totalTime - _currentTime + 0.001);
      }
      ...
      public function advanceTime(param1:Number) : void
      {
         ...
         catch(e:Error)
         {
            ...
            if(_loc2_)
            {
               _loc2_.traceLog(this,repeatCount,_totalTime,_currentTime,new Error(),param1,"cause:",e.getStackTrace());
            }
            else
            {
      ...
      private function _advanceTime(param1:Number) : void
      {
         var _loc5_:* = null;
         var _loc7_:int = 0;
         var _loc8_:* = null;
         var _loc6_:* = null;
         var _loc9_:* = null;
         if(param1 == 0 || isComplete)
         {
            return;
         }
         if(_repeatCount == 0)
         {
            _loc5_ = _target as DisplayObject;
            do
            {
               if(!_loc5_.visible)
               {
         ...
         var _loc11_:* = Number(param1 > _loc2_ ? param1 - _loc2_ : 0);
         _currentTime += param1;
         if(_currentTime <= 0)
         {
            return;
         ...
         var _loc12_:Number = _currentTime / _totalTime;
         var _loc10_:Boolean = _reverse && _currentCycle % 2 == 1;
         var _loc4_:int = _startValues.length;
         _progress = !!_loc10_ ? _transitionFunc(1 - _loc12_) : _transitionFunc(_loc12_);
         _loc7_ = 0;
         while(_loc7_ < _loc4_)
         {
            if(_startValues[_loc7_] != _startValues[_loc7_])
            {
               _startValues[_loc7_] = _target[_properties[_loc7_]] as Number;
            }
            (_loc8_ = _updateFuncs[_loc7_] as Function)(_properties[_loc7_],_startValues[_loc7_],_endValues[_loc7_]);
            _loc7_++;
         }
         if(_onUpdate != null)
         {
         ...
         if(_loc3_ < _totalTime && _currentTime >= _totalTime)
         {
            ...
            {
               _loc6_ = _onComplete;
               _loc9_ = _onCompleteArgs;
               dispatchEventWith("removeFromJuggler");
               if(_loc6_ != null)
               {
                  _loc6_.apply(this,_loc9_);
               }
               if(_currentTime == 0)
               {
                  _loc11_ = 0;
      ...
      {
         updateAngle(3.141592653589793,param1,param2 * 0.017453292519943295,param3 * 0.017453292519943295);
      }
