package feathers.controls.supportClasses
{
   ...
   public class BaseScreenNavigator extends FeathersControl
   {
      ...
      public function getScreenIDs(param1:Vector.<String> = null) : Vector.<String>
      {
         ...
         var _loc3_:int = 0;
         for(param1[_loc3_] in this._screens)
         {
            _loc3_++;
         }
         return param1;
      ...
      protected function autoSizeIfNeeded() : Boolean
      {
         ...
         if(_loc3_)
         {
            if(_loc9_)
            {
               if(this._activeScreen !== null)
               {
                  _loc2_ = Number(this._activeScreen.width);
               }
               else
               {
                  _loc2_ = 0;
            ...
            {
               _loc2_ = Number(this.stage.stageWidth);
            }
         }
         ...
         if(_loc8_)
         {
            if(_loc9_)
            {
               if(this._activeScreen !== null)
               {
                  _loc5_ = Number(this._activeScreen.height);
               }
               else
               {
                  _loc5_ = 0;
         ...
         if(_loc4_)
         {
            if(_loc9_)
            {
               if(_loc7_ !== null)
               {
                  _loc1_ = Number(_loc7_.minWidth);
               }
               else if(this._activeScreen !== null)
               {
                  _loc1_ = Number(this._activeScreen.width);
               }
               else
               {
                  _loc1_ = 0;
         ...
         if(_loc10_)
         {
            if(_loc9_)
            {
               if(_loc7_ !== null)
               {
                  _loc6_ = Number(_loc7_.minHeight);
               }
               else if(this._activeScreen !== null)
               {
                  _loc6_ = Number(this._activeScreen.height);
               }
               else
               {
                  _loc6_ = 0;
            ...
            {
               _loc6_ = Number(this.stage.stageHeight);
            }
         }
      ...
      protected function showScreenInternal(param1:String, param2:Function, param3:Object = null) : DisplayObject
      {
         ...
         var _loc11_:*;
         if(this._validationQueue && !(_loc11_ = this._validationQueue)._isValidating)
         {
         ...
         else
         {
            ...
            if(param2 !== null && param2 !== defaultTransition)
            {
      ...
      protected function transitionComplete(param1:Boolean = false) : void
      {
         ...
         if(param1)
         {
            if(this._activeScreen !== null)
            {
               ...
               this.cleanupActiveScreen();
               this.screenContainer.removeChild(this._activeScreen,_loc2_.canDispose);
         ...
         else
         {
            ...
            _loc9_ = this._previousScreenInTransition;
            _loc8_ = this._previousScreenInTransitionID;
            _loc2_ = IScreenNavigatorItem(this._screens[_loc8_]);
            this._previousScreenInTransition = null;
            this._previousScreenInTransitionID = null;
            if(_loc9_ !== null)
            {
               _loc9_.dispatchEventWith("transitionOutComplete");
            }
            if(_loc3_ !== null)
            {
            ...
            if(_loc9_ !== null)
            {
               if(_loc9_ is IScreen)
               {
                  (_loc5_ = IScreen(_loc9_)).screenID = null;
                  _loc5_.owner = null;
               }
               _loc9_.removeEventListener("resize",activeScreen_resizeHandler);
               this.screenContainer.removeChild(_loc9_,_loc2_.canDispose);
            }
         }
         this._isTransitionActive = false;
         var _loc6_:Function = this._nextScreenTransition;
         this._nextScreenTransition = null;
         if(this._clearAfterTransition)
         {
            this._clearAfterTransition = false;
            this.clearScreenInternal(_loc6_);
         }
         else if(this._nextScreenID !== null)
         {
            _loc7_ = this._nextScreenID;
            this._nextScreenID = null;
            this.showScreenInternal(_loc7_,_loc6_);
         }
      }
