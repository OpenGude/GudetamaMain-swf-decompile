package feathers.controls
{
   ...
   public class Alert extends Panel
   {
      ...
      public function Alert()
      {
         ...
         this.addEventListener("addedToStage",alert_addedToStageHandler);
      }
      ...
      override protected function autoSizeIfNeeded() : Boolean
      {
         ...
         if(_loc12_)
         {
            if(this._measureViewPort)
            {
               _loc4_ = Number(this._viewPort.visibleHeight);
            }
            else
            {
               _loc4_ = 0;
            ...
            {
               _loc4_ = Number(this.currentBackgroundSkin.height);
            }
         }
         if(_loc10_)
         {
            if(this._measureViewPort)
            {
               _loc9_ = Number(this._viewPort.minVisibleWidth);
            }
            else
            {
               _loc9_ = 0;
            ...
            switch(_loc6_)
            {
               default:
                  ...
                  {
                     _loc9_ = Number(_loc6_.minWidth);
                  }
                  break;
               case null:
                  if(this._explicitBackgroundMinWidth > _loc9_)
                  {
                     _loc9_ = Number(this._explicitBackgroundMinWidth);
                     break;
                  }
                  break;
               case null:
         ...
         if(_loc14_)
         {
            if(this._measureViewPort)
            {
               _loc11_ = Number(this._viewPort.minVisibleHeight);
            }
            else
            {
               _loc11_ = 0;
            ...
            switch(_loc6_)
            {
               default:
                  ...
                  {
                     _loc11_ = Number(_loc6_.minHeight);
                  }
                  break;
               case null:
                  if(this._explicitBackgroundMinHeight > _loc11_)
                  {
                     _loc11_ = Number(this._explicitBackgroundMinHeight);
                     break;
                  }
                  break;
               case null:
      ...
      protected function closeAlert(param1:Object) : void
      {
         ...
         this.dispatchEventWith("close",false,param1);
         this.dispose();
      }
      ...
      protected function alert_addedToStageHandler(param1:Event) : void
      {
         ...
         _loc2_.nativeStage.addEventListener("keyDown",alert_nativeStage_keyDownHandler,false,_loc3_,true);
         this.addEventListener("removedFromStage",alert_removedFromStageHandler);
      }
      
      protected function alert_removedFromStageHandler(param1:Event) : void
      {
         this.removeEventListener("removedFromStage",alert_removedFromStageHandler);
         var _loc3_:*;
         var _loc2_:Starling = this.stage !== null ? this.stage.starling : (_loc3_ = Starling, starling.core.Starling.sCurrent);
         _loc2_.nativeStage.removeEventListener("keyDown",alert_nativeStage_keyDownHandler);
      }
      ...
      protected function alert_nativeStage_keyDownHandler(param1:KeyboardEvent) : void
      {
         ...
         if(this._acceptButtonIndex !== -1 && _loc2_ === 13)
         {
         ...
         if(this._cancelButtonIndex !== -1 && (_loc2_ === 16777238 || _loc2_ === 27))
         {
            param1.preventDefault();
