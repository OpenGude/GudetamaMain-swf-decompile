package feathers.controls
{
   ...
   public class PageIndicator extends FeathersControl
   {
      ...
      protected function refreshSymbols(param1:Boolean) : void
      {
         ...
         if(param1)
         {
            _loc5_ = this.unselectedSymbols.length;
            _loc4_ = 0;
            while(_loc4_ < _loc5_)
            {
      ...
      protected function touchHandler(param1:TouchEvent) : void
      {
         ...
         if(this.touchPointID >= 0)
         {
            ...
            _loc4_.getLocation(this.stage,_loc7_);
            _loc3_ = this.contains(this.stage.hitTest(_loc7_));
            if(_loc3_)
            {
               _loc8_ = this._pageCount - 1;
               this.globalToLocal(_loc7_,_loc7_);
               if(this._direction == "vertical")
               {
                  if(this._interactionMode === "precise")
                  {
                     _loc2_ = this.selectedSymbol.height + (this.unselectedSymbols[0].height + this._gap) * _loc8_;
                     if((_loc5_ = int(Math.round(_loc8_ * (_loc7_.y - this.symbols[0].y) / _loc2_))) < 0)
                     {
                        _loc5_ = 0;
                     }
                  ...
                  {
                     if(_loc7_.y < this.selectedSymbol.y)
                     {
                        this.selectedIndex = Math.max(0,this._selectedIndex - 1);
                     }
                     if(_loc7_.y > this.selectedSymbol.y + this.selectedSymbol.height)
                     {
                        this.selectedIndex = Math.min(_loc8_,this._selectedIndex + 1);
                     }
                  }
               }
               else if(this._interactionMode === "precise")
               {
                  _loc6_ = this.selectedSymbol.width + (this.unselectedSymbols[0].width + this._gap) * _loc8_;
                  if((_loc5_ = int(Math.round(_loc8_ * (_loc7_.x - this.symbols[0].x) / _loc6_))) < 0)
                  {
                     _loc5_ = 0;
                  }
               ...
               {
                  if(_loc7_.x < this.selectedSymbol.x)
                  {
                     this.selectedIndex = Math.max(0,this._selectedIndex - 1);
                  }
                  if(_loc7_.x > this.selectedSymbol.x + this.selectedSymbol.width)
                  {
                     this.selectedIndex = Math.min(_loc8_,this._selectedIndex + 1);
                  }
               }
