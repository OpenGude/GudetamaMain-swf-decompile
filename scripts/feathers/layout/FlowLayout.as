package feathers.layout
{
   ...
   public class FlowLayout extends EventDispatcher implements IVariableVirtualLayout
   {
      ...
      protected var _offsetPaintHeight:Number = 0;
      ...
      public function set offsetPaintHeight(param1:Number) : void
      {
         _offsetPaintHeight = param1;
      }
      ...
      public function layout(param1:Vector.<DisplayObject>, param2:ViewPortBounds = null, param3:LayoutBoundsResult = null) : LayoutBoundsResult
      {
         ...
         var _loc36_:Number = !!param2 ? param2.x : 0;
         var _loc37_:Number = !!param2 ? param2.y : 0;
         ...
         var _loc34_:Number = !!param2 ? param2.explicitWidth : NaN;
         var _loc39_:Number = !!param2 ? param2.explicitHeight : NaN;
         var _loc23_:* = _loc34_ !== _loc34_;
         ...
         var _loc12_:Number = _loc37_ + this._paddingTop;
         var _loc15_:* = 0;
         var _loc21_:* = 0;
         var _loc9_:Number = this._verticalGap;
         var _loc26_:* = this._firstHorizontalGap === this._firstHorizontalGap;
         var _loc40_:* = this._lastHorizontalGap === this._lastHorizontalGap;
         var _loc19_:int = _loc32_ - 2;
         do
         {
            if(_loc27_ > 0)
            {
            ...
            _loc21_ = Number(!!this._useVirtualLayout ? _loc25_ : 0);
            _loc10_ = _loc36_ + this._paddingLeft;
            this._rowItems.length = 0;
            _loc30_ = 0;
            _loc35_ = 0;
            for(; _loc27_ < _loc32_; _loc27_++)
            {
               _loc22_ = param1[_loc27_];
               _loc35_ = Number(this._horizontalGap);
               if(_loc26_ && _loc27_ === 0)
               {
                  _loc35_ = Number(this._firstHorizontalGap);
               }
               else if(_loc40_ && _loc27_ > 0 && _loc27_ == _loc19_)
               {
                  _loc35_ = Number(this._lastHorizontalGap);
               }
               if(this._useVirtualLayout && this._hasVariableItemDimensions)
               {
                  _loc33_ = this._widthCache[_loc27_];
                  _loc16_ = this._heightCache[_loc27_];
               }
               ...
               else
               {
                  ...
                  _loc17_ = Number(_loc22_.width);
                  _loc7_ = Number(_loc22_.height);
                  if(this._useVirtualLayout)
                  {
                     if(this._hasVariableItemDimensions)
                     {
               ...
               if(_loc20_ && _loc30_ > 0 && _loc10_ + _loc17_ > _loc24_ - this._paddingRight)
               {
                  _loc41_ = _loc27_ - 1;
                  _loc35_ = Number(this._horizontalGap);
                  if(_loc26_ && _loc41_ === 0)
                  {
                     _loc35_ = Number(this._firstHorizontalGap);
                  }
                  else if(_loc40_ && _loc41_ > 0 && _loc41_ == _loc19_)
                  {
                     _loc35_ = Number(this._lastHorizontalGap);
                  }
                  break;
            ...
            if((_loc8_ = _loc10_ - _loc35_ + this._paddingRight - _loc36_) > _loc15_)
            {
               _loc15_ = _loc8_;
            }
            _loc30_ = this._rowItems.length;
            if(_loc20_)
            {
               _loc38_ = 0;
               if(this._horizontalAlign === "right")
               {
                  _loc38_ = Number(_loc24_ - _loc8_);
               }
               else if(this._horizontalAlign === "center")
               {
                  _loc38_ = Number(Math.round((_loc24_ - _loc8_) / 2));
               }
               if(_loc38_ !== 0)
               {
                  _loc29_ = 0;
         ...
         while(_loc27_ < _loc32_);
         
         this._rowItems.length = 0;
         if(_loc20_ && (_loc23_ || _loc34_ < _loc15_))
         {
            if((_loc42_ = _loc15_) < _loc28_)
            {
               _loc42_ = _loc28_;
            }
            else if(_loc42_ > _loc18_)
            {
               _loc42_ = _loc18_;
            }
            _loc38_ = 0;
            if(this._horizontalAlign === "right")
            {
               _loc38_ = Number(_loc24_ - _loc42_);
            }
            else if(this._horizontalAlign === "center")
            {
               _loc38_ = Number(Math.round((_loc24_ - _loc42_) / 2));
            }
            if(_loc38_ !== 0)
            {
               _loc27_ = 0;
      ...
      public function measureViewPort(param1:int, param2:ViewPortBounds = null, param3:Point = null) : Point
      {
         ...
         do
         {
            ...
            _loc19_ = Number(!!this._useVirtualLayout ? _loc21_ : 0);
            _loc9_ = _loc30_ + this._paddingLeft;
            _loc25_ = 0;
            _loc29_ = 0;
            while(_loc23_ < param1)
            {
               _loc29_ = Number(this._horizontalGap);
               if(_loc22_ && _loc23_ === 0)
               {
                  _loc29_ = Number(this._firstHorizontalGap);
               }
               else if(_loc33_ && _loc23_ > 0 && _loc23_ == _loc17_)
               {
                  _loc29_ = Number(this._lastHorizontalGap);
               }
               if(this._hasVariableItemDimensions)
               {
                  _loc27_ = this._widthCache[_loc23_];
                  _loc14_ = this._heightCache[_loc23_];
                  if(_loc27_ !== _loc27_)
                  {
                     _loc15_ = _loc26_;
                  }
      ...
      public function getVisibleIndicesAtScrollPosition(param1:Number, param2:Number, param3:Number, param4:Number, param5:int, param6:Vector.<int> = null) : Vector.<int>
      {
         ...
         var _loc10_:Number = param2 + param4 + _offsetPaintHeight;
         ...
         do
         {
            ...
            while(_loc14_ < param5)
            {
               ...
               if(this._hasVariableItemDimensions)
               {
                  _loc19_ = this._widthCache[_loc14_];
                  _loc22_ = this._heightCache[_loc14_];
               }
               if(this._hasVariableItemDimensions)
               {
                  if(_loc19_ !== _loc19_)
      ...
      protected function calculateMaxScrollYAndRowHeightOfIndex(param1:int, param2:Vector.<DisplayObject>, param3:Number, param4:Number, param5:Number, param6:Number, param7:Point = null) : Point
      {
         ...
         while(!_loc21_)
         {
            ...
            _loc8_ = Number(!!this._useVirtualLayout ? _loc10_ : 0);
            ...
            for(; _loc12_ < _loc16_; _loc12_++)
            {
               ...
               if(this._useVirtualLayout && this._hasVariableItemDimensions)
               {
                  _loc18_ = this._widthCache[_loc12_];
                  _loc22_ = this._heightCache[_loc12_];
               }
